...
def read_and_preprocess_with_augment(image_bytes, bucket_name=None, product_name=None, description=None):
    return read_and_preprocess(image_bytes, bucket_name, product_name, description, augment=True)
    
def read_and_preprocess(image_bytes, bucket_name=None, product_name=None, description=None, augment=False):
...
def make_input_fn(csv_of_filenames, batch_size, mode, augment = False):
    def _input_fn():
        def decode_csv(csv_row):
            bucket_name, product_id, product_name, description = tf.decode_csv(
                records = csv_row,
                record_defaults = [[''],[''],[''],['']],
                use_quote_delim=True)
            image_bytes = tf.read_file(filename = product_id)
            return image_bytes, bucket_name, product_name, description
...
#     model_function = cnn_model[hparams['model']]
    
    # Build selected Keras model
    model = cnn_model(input_tensor, hparams)
